...
If a pane object includes `cmd`, the value SHALL override the default command the template would run for that pane. There is no window-level `cmd` fallback.
...
- **THEN** the generated script runs `tail -f app.log` for that pane instead of the template default

#### Scenario: Absent cmd leaves template default intact
- **GIVEN** a pane with no `cmd` field
- **WHEN** template variables are derived
- **THEN** `cmd_PN` for that pane is an empty string, and templates use their default command
